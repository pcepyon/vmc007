...
class TestStatusAPI:
    ...
    @patch('data_ingestion.api.views.get_job_status')
    def test_status_endpoint_returns_job_data(self, mock_get_status):
        """
        GIVEN a valid job_id
        WHEN GET /api/upload/status/{job_id}/
        THEN returns job status data
        """
        # Arrange
        ...
        # Act
        response = self.client.get('/api/upload/status/test-uuid/')
    ...
    @patch('data_ingestion.api.views.get_job_status')
    def test_status_invalid_job_id_returns_404(self, mock_get_status):
        """
        GIVEN an invalid job_id
        WHEN GET /api/upload/status/{job_id}/
        ...
        # Act
        response = self.client.get('/api/upload/status/invalid-uuid/')
