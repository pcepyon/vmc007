...
- [환경 변수 설정](#환경-변수-설정)
- [API 문서](#api-문서)
- [테스트](#테스트) 📘 [상세 가이드](./docs/TESTING.md)
- [로그 및 디버깅](#로그-및-디버깅)
- [배포](#배포)
- [개발 가이드](#개발-가이드)
- [트러블슈팅](#트러블슈팅)
...
```bash
# 프로젝트 루트에서
./dev-start.sh   # 백엔드 + 프론트엔드 동시 시작
```

서버가 자동으로 시작되며 다음 URL로 접속 가능합니다:
...
> 📘 **상세한 테스트 가이드**: [docs/TESTING.md](./docs/TESTING.md)

### 테스트 현황 요약

| 구분 | 파일 수 | 테스트 케이스 | 커버리지 목표 | 프레임워크 |
|------|---------|--------------|--------------|-----------|
| **Backend** | 24개 | 248개 | 95% | pytest + pytest-django |
| **Frontend** | 16개 | 88개+ | 50% | Jest + React Testing Library |
| **E2E** | 1개 | 4개 | - | Playwright |
...
```bash
# 에러 로그만 보기
grep -i error .dev/backend.log

# 특정 API 엔드포인트 요청 찾기
grep "POST /api/upload" .dev/backend.log

# 최근 50줄 보기
tail -50 .dev/backend.log
```
...
[02/Nov/2025 21:04:58] "GET /api/dashboard/filter-options/ HTTP/1.1" 200 352
[02/Nov/2025 21:05:12] "POST /api/upload/ HTTP/1.1" 202 89
[02/Nov/2025 21:05:15] "GET /api/upload/status/abc123/ HTTP/1.1" 200 145
```

에러 발생 시:
```
[02/Nov/2025 21:10:23] "POST /api/upload/ HTTP/1.1" 400 78
ERROR: Invalid file format: Expected CSV/Excel
```
...
# 실행 중인 서버 PID 확인
cat .dev/backend.pid
cat .dev/frontend.pid

# 프로세스가 살아있는지 확인
ps -p $(cat .dev/backend.pid)
ps -p $(cat .dev/frontend.pid)
...
```bash
# 서버가 실제로 실행 중인지 확인
ps -p $(cat .dev/backend.pid 2>/dev/null) || echo "서버가 실행 중이지 않습니다"
